...
func listCmdCtr(m *tb.Message) {
	...
	if mention != "" {
		...
		if err != nil {
			B.Send(m.Chat, fmt.Sprintf("内部錯誤 list@1"))
			return
		}

		subSourceMap, err := user.GetSubSourceMap()
		if err != nil {
			B.Send(m.Chat, fmt.Sprintf("内部錯誤 list@2"))
			return
		}
...
func setFeedItemBtnCtr(c *tb.Callback) {
	...
	if err != nil {
		_, _ = B.Edit(c.Message, "用户未訂閱該rss，錯誤代碼02。")
		return
	}
...
func setSubTagBtnCtr(c *tb.Callback) {
	...
	msg := fmt.Sprintf(
		"請使用`/setfeedtag %d tags`命令為該訂閱設置標籤，tags為需要設置的標籤，以空格分隔。（最多設置三个標籤） \n"+
...
func unsubCmdCtr(m *tb.Message) {
	...
	if mention == "" {
		if url != "" {
			...
			if err != nil {
				errorCtr(m, "Bot錯誤，請聯繫管理員！錯誤代碼01")
				return
			}
			...
			if len(subs) > 0 {
				...
				for _, sub := range subs {
					...
					if err != nil {
						errorCtr(m, "Bot錯誤，請聯繫管理員！錯誤代碼02")
						return
					}
...
/export 導出 OPML 文件
/unsuball 取消所有訂閱
詳細使用方法請看：https://github.com/makubex2010/RSS_BOT
`
...
func setIntervalCmdCtr(m *tb.Message) {
	...
	if len(args) < 1 {
		_, _ = B.Send(m.Chat, "/setinterval [interval] [sub id] 訂閱標籤刷新频率（可設置多个sub id，以空格分割）")
		return
	}

	interval, err := strconv.Atoi(args[0])
	if interval <= 0 || err != nil {
		_, _ = B.Send(m.Chat, "請輸入正確的抓取频率")
		return
	}
	...
	case fsm.Sub:
		{
			...
			if !CheckURL(url[0]) {
				_, _ = B.Send(m.Chat, "請回覆正確的URL", &tb.ReplyMarkup{ForceReply: true})
				return
			}
...
func docCtr(m *tb.Message) {
	...
	if err != nil {
		if err.Error() == "fetch opml file error" {
			_, _ = B.Send(m.Chat,
				"下载 OPML 文件失敗，請檢查 bot 服務器能否正常連接至 Telegram 服務器或稍後嘗試導入。錯誤代碼 02")
